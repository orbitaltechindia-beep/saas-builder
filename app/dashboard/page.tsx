...
export default function ClientDashboard() {
  ...
  const handleSelectTemplate = async (templateId: string) => {
    ...
    await setDoc(doc(db, 'sites', newSiteId), { 
      pageData: template ? template.pageData : gTemplate.pageData, 
      title: name,
      status: 'live', 
      pages: corePages,
      pageData_resources: [],
      ...
      <main className="flex-1 p-10 overflow-y-auto">
        ...
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl shadow-lg p-6 mb-12 text-white max-w-2xl">
          ...
          <input 
            ...
            className="w-full bg-white/20 backdrop-blur text-white placeholder-blue-100 p-3 rounded-lg outline-none focus:bg-white/30 mb-4"
          />
          
          {/* Module Selection */}
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-4">
            {['Courses', 'Centres', 'Results', 'Testimonials', 'Blogs', 'Enquiry CTA'].map(module => (
              <label key={module} className="flex items-center gap-2 text-sm cursor-pointer bg-white/10 px-3 py-2 rounded-lg">
                <input 
                  type="checkbox" 
